...
#define SIFT_MATCH_DISTANCE_RATIO 0.5														//threshold on squared ratio of distances between NN and 2nd NN	 (Lowe uses 0.8, RobHess uses 0.49)	

MySIFT::MySIFT(vector<Image>* imgs, int height, int width) :
	s(3), baseSigma(1.6f), contrastThreshold(0.04f), curvaturesThreshold(10.0f)
{
	images = imgs;
...
bool MySIFT::adjustLocalExtrema(int ith, int r, int c, int theOctave, int theLayer, int rows, int cols)
{
	...
	if (abs(accurate_local_extrema)*s < contrastThreshold) //Lowe uses 0.03, but Rob Hess use 0.04/s
		return false;
		...
		for (int q = -radius_of_descriptor; q <= radius_of_descriptor; q++)
		{
			...
			if (i_bin > -1 && i_bin < SIFT_DESCR_HIST_ARRAY_WIDTH && j_bin > -1 && j_bin < SIFT_DESCR_HIST_ARRAY_WIDTH)
			{
				...
				*/
				...
				//Make sure ob is in 0 - 7 dimension
...
void MySIFT::matching()
{
	...
	for (int i = 0; i < (*images).size(); i++)
	{
		...
		Index<cvflann::L2_Simple<float> > kdtree(trainDescriptors, KDTreeIndexParams(4));
		kdtree.buildIndex();
